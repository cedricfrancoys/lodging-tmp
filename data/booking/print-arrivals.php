...
list($params, $providers) = announce([
    ...
    'params'        => [
        ...
        /*
        // #memo - these values are fetched from the 'params' parameter
        'date_from' => [
            'type'              => 'date',
...
// if no center is provided, fallback to current users' (first) default
...
// create domain to retrieve all bookings that match the given date range
$domain = new Domain([
        ['date_from', '>=', $params['date_from']], ['date_from', '<=', $params['date_to']]
    ]);

// limit domain to given center and confirmed bookings
$domain
    ->addCondition(new DomainCondition('center_id', '=', $params['center_id']))
    ->addCondition(new DomainCondition('status', 'in', ['confirmed', 'validated']));

// retrieve the listing of matching Booking objects
$bookings_ids = Booking::search($domain->toArray(), ['sort'  => ['date_from' => 'asc']])->ids();
