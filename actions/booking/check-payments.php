...
list($params, $providers) = announce([
    'description'   => "Checks if all due payments have been received for a given booking.",
    'params'        => [
        'id' =>  [
...
$booking = Booking::id($params['id'])->read(['id', 'name', 'center_office_id', 'fundings_ids'])->first(true);

if(!$booking) {
    throw new Exception("unknown_booking", QN_ERROR_UNKNOWN_OBJECT);
}

$fundings = Funding::ids($booking['fundings_ids'])->read(['booking_id', 'due_date', 'is_paid'])->get();
...
if($fundings) {
    ...
    foreach($fundings as $fid => $funding) {
        if($funding['booking_id'] != $booking['id'] && $funding['due_date'] <= $today && !$funding['is_paid']) {
            $is_paid = false;
            $result[] = $fid;
            break;
        }
...
if(!$is_paid) {
    ...
    $dispatch->dispatch('lodging.booking.payments', 'lodging\sale\booking\Booking', $params['id'], 'warning', 'lodging_booking_check-payments', ['id' => $params['id']], [], null, $booking['center_office_id']);
