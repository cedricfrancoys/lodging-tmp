...
export class AppRootComponent implements OnInit {
    ...
    public async ngOnInit() {
        console.debug('AppRootComponent::Bootstrapping Booking App');
        ...
        const left_menu:any = await this.api.getMenu('lodging', 'booking.left');
        this.navMenuItems = left_menu.items;
        this.translationsMenuLeft = left_menu.translation;

        const top_menu:any = await this.api.getMenu('lodging', 'booking.top');
            ...
            context: {
                ...
                callback:   (data:any) => {
                    if(data && data.objects && data.objects.length) {
                        console.debug('received value from create booking', data);
