...
export class BookingFundingComponent implements OnInit, AfterContentInit {
  ...
  private async loadCustomer() {
    const result:Array<any> = <Array<any>> await this.api.read("lodging\\sale\\customer\\Customer", [this.booking.customer_id], Object.getOwnPropertyNames(new Customer()));
    if(result && result.length) {
      const item:any = result[0];
  ...
  private async loadPayer() {
    const result:Array<any> = <Array<any>> await this.api.read("lodging\\sale\\customer\\Customer", [this.booking.payer_organisation_id], Object.getOwnPropertyNames(new Customer()));
    if(result && result.length) {
      const item:any = result[0];
      let payer:any = new Customer();
      for(let field of Object.getOwnPropertyNames(payer) ) {
        if(item.hasOwnProperty(field)) {
          payer[field] = item[field];
        }
      }
      this.payer = <Customer> payer;
    }
  }
